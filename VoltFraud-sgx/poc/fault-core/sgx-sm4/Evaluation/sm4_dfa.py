...
def check(output, encrypt=None, verbose=2, init=False, _intern={}):
    """
...
fault_round_30 = []
fault_round_31 = []
def collect_fault_ct(ref_ct):
    fault_ct = []
    _, index = check(bytes.fromhex(ref_ct), init=True)
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    target_dir = os.path.join(BASE_DIR, "../")
    dirs = ['../logs-3', '../logs-1', '../logs-2', '../']
    ...
    for dir in dirs:
        ...
        for fname in os.listdir(target_dir):
            ...
            for i in range(len(lines) - 2):
                ...
                # match
                if ("difference found at block" in line1):
                    fault_ct = bytes(int(x, 16) for x in line2.split())
                    all_fault_ct.append(fault_ct)
                    status, index = check(fault_ct)
                    if status == FaultStatus.round31Fault and fault_ct not in fault_round_31:
                        fault_round_31.append(fault_ct)
                    elif status == FaultStatus.round30Fault and fault_ct not in fault_round_30:
                        fault_round_30.append(fault_ct)
                    elif status == FaultStatus.round29Fault and fault_ct not in fault_round_29:
                        fault_round_29.append(fault_ct)
                    else:
                        pass
...
if __name__ == "__main__":
    ref_ct = 'f5b1c448c2354e6d499ab12826b89bca'
    collect_fault_ct(ref_ct)
    print(f"fault_round_31: {len(fault_round_31)}")
    print(f"fault_round_30: {len(fault_round_30)}")
    print(f"fault_round_29: {len(fault_round_29)}")
    print(f"fault_round_28: {len(fault_round_28)}")
    last_round_key = []
    for i in range(4):
        last_round_key.append(crack_round(list(set(all_fault_ct)), bytes.fromhex(ref_ct), last_round_key))

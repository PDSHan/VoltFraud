...
void usage(void)
{
	printf("Usage: glitch_controller -b <bps> -p <serialport> [OPTIONS]\n"
			...
			"  -c  --channel			  DC power output channel\n"
			"  -g  --glitch               Do the voltage glitch\n"
            "  -d  --delay                Delay after trigger (Td) in S"
			"      --iter		      	  Iterations of undervolting (assembly)\n"
			"      --pre_volt	          Voltage before the glitch (Vp)\n"
			"      --pre_width	          Delay before the glitch (TTp)\n"
			"      --fault_voltage	      Glitch voltage (Vf)\n"
			// "      --rst_volt	          Reset voltage (Vn)\n"
			"      --fault_width	          Reset delay (TTf)\n"
...
int main(int argc, char *argv[])
{
	...
	static struct option loptions[] = {
		...
		{"pstart",      no_argument,       0, 'p'},
		{"delay",      required_argument,       0, 'd'},

		{"iter",      required_argument, &command_flag , OPTION_ITERATIONS},
		{"pre_volt",      required_argument, &command_flag , OPTION_PRE_VOLTAGE },
		{"pre_width",      required_argument, &command_flag, OPTION_PRE_DELAY },
		{"fault_volt",      required_argument, &command_flag, OPTION_GLITCH_VOLTAGE },
		...
		{"fault_width",      required_argument, &command_flag, OPTION_RST_DELAY },
	...
	char * target_name = 0;
	while(1) {
		opt = getopt_long (argc, argv, "hqpt::gc:d:",
		...
		switch (opt) {
			case 0:
				switch(command_flag){
					case OPTION_ITERATIONS:
						...
						snprintf(log_info, sizeof(log_info), "max_iterations %lu\n", calc_info->max_iterations);
						write(log_fd, log_info, strlen(log_info));
						break;
					case OPTION_RETRIES:
						retries = strtol(optarg,NULL,10);
						snprintf(log_info, sizeof(log_info), "retries %li\n", retries);
						write(log_fd, log_info, strlen(log_info));
						break;
					case OPTION_PRE_VOLTAGE:
						undr_info->pre_voltage =  strtod(optarg, &endptr);
						snprintf(log_info, sizeof(log_info), "pre_volt %.4f\n", undr_info->pre_voltage);
						write(log_fd, log_info, strlen(log_info));
						break;
					case OPTION_PRE_DELAY:
						undr_info->pre_delay = strtod(optarg, &endptr);
						snprintf(log_info, sizeof(log_info), "pre_width  %.6f\n", undr_info->pre_delay);
						write(log_fd, log_info, strlen(log_info));
						break;
					case OPTION_GLITCH_VOLTAGE:
						undr_info->glitch_voltage = strtod(optarg, &endptr);
						snprintf(log_info, sizeof(log_info), "fault_volt %.4f\n", undr_info->glitch_voltage);
						write(log_fd, log_info, strlen(log_info));
						break;
					// case OPTION_RST_VOLTAGE:
					// 	undr_info->rst_voltage =  atof(optarg);
					// 	snprintf(log_info, sizeof(log_info), "rst_voltage %f\n", undr_info->rst_voltage);
					// 	write(log_fd, log_info, strlen(log_info));
					// 	break;
					case OPTION_RST_DELAY:
						undr_info->rst_delay = strtod(optarg, &endptr);
						snprintf(log_info, sizeof(log_info), "fault_width %.6f\n", undr_info->rst_delay);
						write(log_fd, log_info, strlen(log_info));
						break;
					case OPTION_TARGET_ID:
						undr_info->target_func_id = atoi(optarg);
						snprintf(log_info, sizeof(log_info), "target_func_id %d\n", undr_info->target_func_id);
						write(log_fd, log_info, strlen(log_info));
						break;
					...
					case OPTION_CALC_THREAD_NUM:
						...
						snprintf(log_info, sizeof(log_info), "thread %d\n", calc_info->thread_number);
						write(log_fd, log_info, strlen(log_info));
						break;
					case OPTION_SIMPLE:
						snprintf(log_info, sizeof(log_info), "Simple Mode\n");
						write(log_fd, log_info, strlen(log_info));
						flag_simple_mode = 1;
						break;
					case OPTION_START_V:
						undr_info->s_start_v = atof(optarg);
						snprintf(log_info, sizeof(log_info), "Start voltage: %lf", undr_info->s_start_v);
						write(log_fd, log_info, strlen(log_info));
						break;
					case OPTION_STEP_COUNT:
						undr_info->s_step_count = atoi(optarg);
						snprintf(log_info, sizeof(log_info), "Step Count: %d\n", undr_info->s_step_count);
						write(log_fd, log_info, strlen(log_info));
						break;
				}
				break;
			case 'g':
				close_BNC_Arb();
				configure_glitch_with_delay(1,1,undr_info->pre_voltage,0,0,0,0);
				start_DCpower_type1();
				...
				snprintf(log_info, sizeof(log_info), "------Start glitching------\n");
				write(log_fd, log_info, strlen(log_info));

				if(flag_simple_mode) {
					for (int isc = 0; isc < undr_info->s_step_count; isc ++){
						undervolt =  undr_info->pre_voltage + 0.001*isc;
						snprintf(log_info, sizeof(log_info),"under voltage is %fV\n", undervolt);
						write(log_fd, log_info, strlen(log_info));
						for (int repeat = 0; repeat < retries; repeat++ ) {
							int ret=-1;
							if (delay_start > 0) {
								ret = configure_glitch_with_delay(1, channel, undervolt, 0,0,0,delay_start);
							} else {
								ret = configure_glitch_with_delay(1, channel, undervolt,0,0,0,0);
							}
							if(ret < 0)
								exit(EXIT_FAILURE);
							...
							if (faulty_result_found == 1) {
								snprintf(log_info, sizeof(log_info), "Fault at step: %d, Repeat: %d\n", isc, repeat);
								write(log_fd, log_info, strlen(log_info));
					...
					for (int isc = 0; isc < undr_info->s_step_count; isc ++){
						...
						undervolt = undr_info->glitch_voltage + 0.001*isc;
						snprintf(log_info, sizeof(log_info),"pre_volt %.4f, pre_width %.6f, fault_volt %.4f, fault_width %.6f, delay %.6f, \n", undr_info->pre_voltage,undr_info->pre_delay,undervolt, undr_info->rst_delay, delay_start);
						write(log_fd, log_info, strlen(log_info));
						long c_retry;
						close_BNC_Arb();
						configure_glitch_with_delay(1, channel, undr_info->pre_voltage, \
								undr_info->pre_delay , undervolt, undr_info->rst_delay, delay_start);
						...
						for(c_retry=0; c_retry<retries; c_retry++){
							...
							close(fd_trigger);
							printf("mul time: %.6f\n", (end1.tv_sec-start.tv_sec) + (end1.tv_nsec - start.tv_nsec)/1e9);

							if(faulty_result_found == 1){
								snprintf(log_info, sizeof(log_info), "Fault at step: %d, Repeat: %ld\n", isc, c_retry);
								write(log_fd, log_info, strlen(log_info));
								faulty_result_found = 0;
							}
						}
			...
			case 't':
				  ...
				  snprintf(log_info, sizeof(log_info), "timeout set to %d millisecs\n",timeout);
				  write(log_fd, log_info, strlen(log_info));
				  break;
			case 'd':
				char *endptr;
				delay_start = strtod(optarg, &endptr);
				snprintf(log_info, sizeof(log_info), "delay_start %.6f\n", delay_start);
				write(log_fd, log_info, strlen(log_info));
				break;
			case 'h':
				  usage();
			...
			case 'c':
				channel = atoi(optarg);
				snprintf(log_info, sizeof(log_info), "DC power output channel is %d\n", channel);
				write(log_fd, log_info, strlen(log_info));
				break;

...
void memory_read(fault_params *params){
    ...
    // if(base==NULL){
    //     snprintf(log_info, sizeof(log_info), "check_hugepage return null.\n");
    //     write(log_fd, log_info, strlen(log_info));        
    //     data = aligned_alloc(64, ARRAY_SIZE * sizeof(cl64_t));
    // }else{
    //     use_hugepage = 1;
    //     data = (cl64_t *)(base + index * REGION_SIZE);
    // }
    ...
    //     return;
    // }
    cl64_t data[ARRAY_SIZE];
    cl64_t data2[ARRAY_SIZE];
    ...
    for(int i=0; i<1000; i++) {
        ...
        for (int ii=0; ii<params->retries; ii++){
            ...
            // uintptr_t va;
            // uintptr_t paddr;
            // for(va = data_start;va<(data_start+REGION_SIZE);va=va+(1ULL<<12)){
            //     paddr = translate_va2pa(va);
            //     snprintf(log_info, sizeof(log_info), "va: 0x%lx, pa: 0x%lx\n", va, paddr);
            //     write(log_fd, log_info, strlen(log_info));
            // }
